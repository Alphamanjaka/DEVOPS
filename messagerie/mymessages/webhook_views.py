...
def _get_bot_user():
    ...
    if created:
        logger.info("github-bot user created")
    return bot


@csrf_exempt
def github_webhook(request):
    logger.info("github_webhook called method=%s", request.method)

    if request.method == 'GET':
        logger.info("github_webhook GET (health check)")
        return JsonResponse({'message': 'Webhook GitHub: utilisez POST avec un payload GitHub.'})

    secret = os.environ.get('GITHUB_WEBHOOK_SECRET')
    if not secret:
        logger.error("github_webhook: GITHUB_WEBHOOK_SECRET not configured")
    ...
    logger.info("github_webhook event=%s", event)

    try:
        payload = json.loads(body)
    except json.JSONDecodeError:
        logger.error("github_webhook: invalid JSON")
    ...
    if event == 'push':
        ...
        logger.info("github_webhook push: %s/%s by %s (%d commits)", repo_name, branch, pusher, commit_count)
        ...
        logger.info("github_webhook pull_request: %s #%s %s", repo_name, pr.get('number'), action)
    else:
        body_text = f"[GitHub] Événement: {event}\n{json.dumps(payload, indent=2)[:2000]}"
        logger.info("github_webhook other event: %s", event)
    ...
    logger.info("github_webhook done: notified %d users", active_users.count())
